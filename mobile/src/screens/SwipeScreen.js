...
// Arka planda film kuyruğu — global tutulur, ekran yeniden mount olsa bile korunur
...
export default function SwipeScreen({ navigation }) {
  ...
  const loadMovies = async () => {
    setLoading(true);
    // Önce önceden yüklenmiş filmler var mı bak
    if (prefetchedMovies.length > 0) {
      setMovies(prefetchedMovies);
      prefetchedMovies = [];
      setLoading(false);
      prefetchMovies(); // hemen sonrakini hazırla
      return;
    }
    try {
      const res = await api.get('/movies/suggestions');
      ...
      setCurrentIndex(0);
      position.setValue({ x: 0, y: 0 });
      prefetchMovies(); // arka planda sonraki batch'i hazırla
    } catch (err) {
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    // Prefetch hazırsa anında göster
    if (prefetchedMovies.length > 0) {
      setMovies(prefetchedMovies);
      prefetchedMovies = [];
      setCurrentIndex(0);
  ...
  const triggerSwipeUp = () => {
    Animated.timing(position, { toValue: { x: 0, y: -SCREEN_HEIGHT }, duration: 250, useNativeDriver: false }).start(() => {
      position.setValue({ x: 0, y: 0 });
      handleWatchlist();
    });
  };
  ...
  const handleAdd = async () => {
    ...
    // UI'ı hemen ilerlet, API isteği arka planda
    setCurrentIndex((prev) => prev + 1);
    try {
  ...
  if (!currentMovie) return (
    <View style={styles.center}>
      ...
      <Text style={styles.doneText}>Yeni filmler hazırlanıyor...</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.actions}>
        ...
        <TouchableOpacity style={[styles.actionBtn, styles.watchlistBtn]} onPress={triggerSwipeUp}>
          {actionLoading === 'watchlist'
            ? <ActivityIndicator color="#FFD700" size="small" />
            : <Text style={[styles.actionIcon, { fontSize: 20 }]}>📋</Text>
          }
        </TouchableOpacity>
        <TouchableOpacity style={[styles.actionBtn, styles.likeBtn]} onPress={triggerSwipeRight}>
          {actionLoading === 'add'
...
const styles = StyleSheet.create({
  ...
  topRight: { position: 'absolute', top: 12, right: 16, flexDirection: 'row', gap: 8, zIndex: 10 },
  iconBtn: {
  ...
  cardContainer: { flex: 1, alignItems: 'center', justifyContent: 'center', marginHorizontal: 12 },
  card: {
    position: 'absolute', width: SCREEN_WIDTH - 24, height: CARD_HEIGHT,
    borderRadius: 28, overflow: 'hidden', backgroundColor: '#1a1a1a',
    shadowColor: '#000', shadowOffset: { width: 0, height: 8 },
    shadowOpacity: 0.4, shadowRadius: 16, elevation: 12,
  },
  card2: { transform: [{ scale: 0.94 }], opacity: 0.85 },
  card3: { transform: [{ scale: 0.88 }], opacity: 0.5 },
  cardImage: { width: '100%', height: '100%', resizeMode: 'cover' },
  cardGradient: {
    position: 'absolute', bottom: 0, left: 0, right: 0, height: '55%',
    backgroundColor: 'rgba(0,0,0,0.01)', borderBottomLeftRadius: 28, borderBottomRightRadius: 28,
  },
  swipeBadge: {
    position: 'absolute', top: 28, flexDirection: 'row', alignItems: 'center', gap: 6,
    paddingVertical: 8, paddingHorizontal: 18, borderRadius: 12, borderWidth: 3,
  },
  swipeBadgeLike: { left: 20, borderColor: '#00E676', backgroundColor: 'rgba(0,230,118,0.15)', transform: [{ rotate: '-12deg' }] },
  swipeBadgeNope: { right: 20, borderColor: '#FF1744', backgroundColor: 'rgba(255,23,68,0.15)', transform: [{ rotate: '12deg' }] },
  swipeBadgeWatch: { alignSelf: 'center', left: '28%', borderColor: '#FFD600', backgroundColor: 'rgba(255,214,0,0.15)' },
  swipeBadgeText: { color: '#fff', fontWeight: '900', fontSize: 15, letterSpacing: 1 },
  swipeBadgeIcon: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
  cardInfo: { position: 'absolute', bottom: 0, left: 0, right: 0, padding: 20, backgroundColor: 'rgba(0,0,0,0.7)' },
  cardInfoTop: { flexDirection: 'row', gap: 8, marginBottom: 8 },
  ratingPill: {
    backgroundColor: 'rgba(255,214,0,0.2)', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4,
    borderWidth: 1, borderColor: 'rgba(255,214,0,0.4)',
  },
  ratingPillText: { color: '#FFD600', fontSize: 12, fontWeight: 'bold' },
  yearPill: {
    backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4,
  ...
  detailPill: {
    alignSelf: 'flex-start', backgroundColor: 'rgba(229,9,20,0.2)', borderRadius: 20,
    paddingHorizontal: 14, paddingVertical: 6, borderWidth: 1, borderColor: 'rgba(229,9,20,0.5)',
  },
  detailPillText: { color: '#E50914', fontSize: 12, fontWeight: 'bold' },
  actions: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 16, paddingTop: 12, paddingBottom: 8 },
  actionBtn: {
    width: 60, height: 60, borderRadius: 30, justifyContent: 'center', alignItems: 'center',
    shadowColor: '#000', shadowOpacity: 0.3, shadowRadius: 8, elevation: 6,
  },
  nopeBtn: { backgroundColor: '#1a1a1a', borderWidth: 2, borderColor: '#FF1744', width: 54, height: 54, borderRadius: 27 },
  watchlistBtn: { backgroundColor: '#1a1a1a', borderWidth: 2, borderColor: '#FFD600', width: 48, height: 48, borderRadius: 24 },
  ...
  doneText: { color: '#fff', fontSize: 22, fontWeight: 'bold', marginBottom: 8 },
});
